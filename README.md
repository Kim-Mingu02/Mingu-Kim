<div align="center">

# Mingu Kim

**PNU IE 21**  
Industrial Engineering · Optimization · Manufacturing Systems · Data Science

[![Optimization](https://img.shields.io/badge/Optimization-0064FF?style=flat-square)](#professional-interests)
[![Combinatorial Optimization](https://img.shields.io/badge/Combinatorial%20Optimization-1E6BFF?style=flat-square)](#professional-interests)
[![Metaheuristics](https://img.shields.io/badge/Metaheuristics-3182F6?style=flat-square)](#professional-interests)
[![Quantum Optimization](https://img.shields.io/badge/Quantum%20Optimization-4E86FF?style=flat-square)](#professional-interests)

</div>

---

## About

I am an Industrial Engineering student focused on **combinatorial optimization**, **computational optimization**, **metaheuristics**, and **quantum optimization**.  
My academic background spans optimization, operations research, manufacturing systems, simulation, data science, and human factors.

---

## Core Competencies

<table>
  <tr>
    <td><b>Optimization</b></td>
    <td>Optimization Modeling · Operations Research · Metaheuristics · QUBO · Computational Optimization</td>
  </tr>
  <tr>
    <td><b>Manufacturing Systems</b></td>
    <td>Production Systems · Facility Layout · Logistics Systems · Quality Control · Simulation</td>
  </tr>
  <tr>
    <td><b>Data Science & AI</b></td>
    <td>Machine Learning · Data Mining · Deep Learning · Database · Algorithms</td>
  </tr>
  <tr>
    <td><b>Ergonomics</b></td>
    <td>Work Design · Human Factors · Decision Analysis</td>
  </tr>
</table>

---

## Prerequisite & Coursework

<details open>
<summary><b>Basic</b></summary>

- 공학선형대수학
- 공학 미적분학
- 공학통계 1, 2
- 이산수학

</details>

<details open>
<summary><b>Optimization</b></summary>

- 최적화개론
- OR 1, 2
- 최적화 모형과 응용
- 최적화와 계산지능

</details>

<details open>
<summary><b>Manufacturing Systems</b></summary>

- 제조공학
- 생산운영관리
- 공급사슬관리
- 시설계획 및 물류시스템
- 경영정보시스템
- 생산시스템공학
- 창의적 공학설계
- 운영분석
- 시뮬레이션
- 품질관리론

</details>

<details open>
<summary><b>Data Science & AI</b></summary>

- 데이터 구조 및 알고리즘
- 산업데이터과학
- 데이터마이닝
- 데이터베이스
- 딥러닝

</details>

<details open>
<summary><b>Ergonomics</b></summary>

- 작업설계
- 인간공학

</details>

---

## Awards & Research Experience

| Period | Experience | Description |
|---|---|---|
| 2025 | **OptiGrandChallenge Award** | Team Leader, 예공제미남들 |
| 9 months | **Lab Intern, Production Systems Optimization Lab** | Paper writing · Non-stationary time-series forecasting ML task · 2026 기초연구실 핵심연구 C 연구제안서 작성 |

### Research Keywords

`Non-stationary Time Series Forecasting` · `Machine Learning` · `QUBO` · `Combinatorial Optimization` · `Quantum Computing` · `Research Proposal Writing`

---

## Projects

| Project | Area | Keywords |
|---|---|---|
| [시공간 복합데이터를 활용한 전력수요 예측 개선](projects/power-demand-forecasting.md) | Forecasting · Data Science | Spatio-temporal Data · Demand Forecasting · ML |
| [PC방 좌석 레이아웃 개선 시뮬레이션](projects/pc-room-seat-layout-simulation.md) | Simulation · Layout | Facility Layout · Simulation · Customer Flow |
| [Process Capability and SPC Case Study - Craft Beer Brewery in Indiranagar, India](projects/craft-beer-spc-case-study.md) | Quality Engineering | SPC · Process Capability · Quality Control |
| [2-Phase QFD for K2C1 Requirements Analysis](projects/k2c1-qfd-requirements-analysis.md) | QFD · Decision Analysis | QFD · Requirements Analysis · Prioritization |
| [Cobot을 활용한 반품 택배 상자의 커팅 및 재포장 보조 시스템 설계·구현](projects/cobot-return-box-repacking.md) | Manufacturing Automation | Cobot · Human-Robot Collaboration · Process Design |
| [고객 경험 기반 무인 세탁소 공간배치 Layout 최적화](projects/laundromat-layout-optimization.md) | Layout Optimization | Customer Experience · Facility Layout · Optimization |
| [서울 통근 수요 대응을 위한 경기권 환승주차장 최적 입지 선정](projects/commuter-park-and-ride-location.md) | Location Optimization | P-median · Demand Allocation · Transportation |
| [안전벨트 제조공정 이해 및 개선](projects/seatbelt-manufacturing-improvement.md) | Process Improvement | Manufacturing Process · Bottleneck Analysis · Improvement |
| [Roll-On/Roll-Off 선박 수요 배치최적화 및 무게 밸런싱](projects/roro-vessel-loading-optimization.md) | Loading Optimization | Assignment · Weight Balancing · Maritime Logistics |
| [AI 신뢰도와 설명 제공 여부가 금융 투자 의사결정에 미치는 영향 분석](projects/ai-trust-financial-decision-analysis.md) | Human-AI Interaction | AI Trust · Explainability · Decision Making |
| [동해선 단선구간 열차 사고 회복 리스케쥴링](projects/donghae-line-disruption-rescheduling.md) | Rescheduling Optimization | Railway Scheduling · Disruption Recovery · Optimization |

---

## Professional Interests

```text
Combinatorial Optimization
Computational Optimization
Metaheuristics
Quantum Optimization
```

---

<div align="center">

**Focused on building optimization models that connect theory, computation, and real industrial systems.**

</div>
